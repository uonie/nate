...
> **本表保留"源码推论"列，用于事后核对预测准确性**
>
> "源码推论"列由 README §3.2 / §3.3 / §5.1 的源码分析推导而来，属 **[待验证]** 等级，
> 记录于**实测之前**，用途是**给出可证伪的预测**——
> 实测跑出来若与预测不符，说明源码分析有误，必须回溯修正。
>
> "实测"列已按实际数据填写；**仍为 `_未测_` 的格子表示该组合未执行，不得外推。**
> 原始数据见 `results/` 目录。

| 请求停顿 | 实际停顿 | 默认参数（5s）<br>源码推论 [待验证] | 默认参数<br>**实测** | 调优后（60s）<br>源码推论 [待验证] | 调优后<br>**实测** |
|---|---|---|---|---|---|
| 2s | 2.31s | 预期无切主 | ✅ **无切主** | 预期无切主 | _未测_ |
| 3s | 3.84s | 预期无切主 | ✅ **无切主** | 预期无切主 | _未测_ |
| 4s | 4.79s | 预期无切主（PALF 租约走 RPC，不受磁盘影响） | ✅ **无切主** | 预期无切主 | _未测_ |
| 5s | 5.84s | 预期临界（判定条件为 `>` 严格大于） | 🔴 **切主**（4 流中 1 流） | 预期无切主 | _未测_ |
| 6s | 6.32s | 预期切主（日志盘 `has_long_pending_io`） | 🔴 **切主**（4 流中 3 流） | 预期无切主 | _未测_ |
| 8s | 8.08s | 预期切主（日志盘） | 🔴 **切主**（4/4 流） | 预期无切主 | _未测_ |
| 10s | 10.07~10.08s | 预期切主（日志盘；数据盘开始计时） | 🔴 **切主**（4/4 流） | 预期无切主 | ✅ **无切主** |
| 15s | 15.08s | 预期切主（日志盘 + 数据盘 WARNING 临界 10+5） | 🔴 **切主**（4/4 流） | 预期无切主 | ✅ **无切主** |
| 30s | 30.08~30.64s | 预期切主（同上） | 🔴 **切主**（4/4 流） | 预期无切主 | ✅ **无切主** |
| 45s | 45.08s | 预期切主（同上） | _未测_ | 预期无切主 | ✅ **无切主** |
| 60s | 60.07 / 60.84s | 预期切主（同上） | 🔴 **切主**（4/4 流） | 预期临界（`>` 严格大于） | ✅ **无切主**（边界：检测触发但 0.2s 后即解除） |
| 90s | 90.84s | 预期切主（同上） | _未测_ | 预期切主 | 🔴 **切主**（4/4 流，T+61.18s 检出） |
| 120s / 300s+ | — | 预期切主，且可能进入 ERROR | _未测_ | 预期切主 | _未测_ |

> **✅ 源码推论的验证结果：12 个已测格子全部与推论一致，无一例外**
>
> 唯一需要说明的是 **5s 档**：推论为"临界"，实测在 5.844s 实际停顿下**确实切主**
> （判定条件 `now - last_working_time > tolerance_time` 为严格大于，5.844 > 5 成立）。
> 而 4.794s 实际停顿下**零故障事件**。**门槛精确落在 5s，源码分析被完整证实。**
>
> **但推论正确 ≠ 建议正确。** 实测同时发现了源码分析**看不出来**的一件事：
> 见下方 §8.1.1。

### 8.1.1 🔴 实测推翻的一个隐含假设

原表的编排隐含了一个假设：**"无切主"= 好，"切主"= 坏**。
实测数据表明这个假设是错的：

| 实际停顿 | 默认参数（会切主） | 调优后（不切主） | 哪个对业务更好？ |
|---|---|---|---|
| 10s | 写中断 **5s** | 写中断 **9s** | **默认更好** |
| 15s | 写中断 **6s** | 写中断 **15s** | **默认更好** |
| 30s | 写中断 **6s** | 写中断 **29s** | **默认明显更好** |
| 60s | 写中断 **7s** | 写中断 **60s** | **默认压倒性更好** |

**实测公式**：写服务完全中断时长 ≈ `min(磁盘停顿时长, tolerance_time + 约 2s)`

**结论**：切主是 OceanBase 的**恢复手段**，不是故障本身。
"完全吸收"（§8 三条标准）在**业务无感**这个意义上**无法通过调参达成** ——
调参只是把"短而硬的中断 + 状态变更"换成"长而软的阻塞 + 无状态变更"。

详见 README §7.4.1。

### 8.1.2 按本节 §8 三条标准的最终判定

| 分类 | 含义 | 实测落在哪些档 |
|---|---|---|
| **完全吸收** | §8 开头三条标准全部满足 | **无任何一档达成** —— 标准 2（应用侧零报错）在 2s 停顿时即已被打破（`errno 4012`） |
| **部分收敛** | 无切主，但业务侧有报错或 TPS 恢复滞后 | 默认参数 ≤4.8s；调优后 ≤60.8s |
| **无法吸收** | 仍发生切主或停机 | 默认参数 ≥5.8s；调优后 ≥90.8s |

> **🔴 对 Q3 的最终答复**
>
> 原计划中"最关键的一行"是 **6s** —— 预期默认参数切主、调优后不切主。
> **实测完全证实了这个预测**（6.32s：默认切主 3/4 流；调优后无切主）。
>
> **但这一行并不足以回答 Q3。** 真正回答 Q3 的是**跨档位的横向对比**（§8.1.1）：
> 调参确实能消除切主，**但代价是写阻塞时长从"封顶 6~7 秒"变成"等于停顿全长"**。
> 因此 Q3 的答案是：
>
> **"能消除 failover，但不能做到业务无感；且当抖动 ≥60s 时，默认参数对业务反而更友好。"**

### 8.2 副作用记录表 [实测]

调优不能只看"切主没了"，还要看代价。**本表已由实测填写：**

| 观测项 | 默认参数（实测） | 调优后（实测） | 判读 |
|---|---|---|---|
| **稳态 TPS（无注入，150s 采样）** | **789.9**（中位 737.1） | **790.5**（中位 715.0） | ✅ 差异 +0.08%，**无稳态代价** |
| **稳态 p95 延迟（中位）** | **11.65 ms** | **11.24 ms** | ✅ 差异 −3.5%，在噪声内 |
| **写服务完全中断时长（30s 停顿）** | **6s** | **29s** | 🔴 **调优后反而更长**（核心权衡） |
| **抖动结束到 TPS 回基线** | 切主后约 5~7s 恢复，但**降级到基线 17~37% 并持续到停顿结束**，随后 leader 回迁引发**二次扰动** | I/O 恢复后 **1~2s 内直接回满基线**，无降级尾巴、无二次扰动 | 各有优劣：默认恢复早但不干净；调优恢复晚但干净 |
| **抖动期间业务错误数** | 8.1s 停顿 → **8**；30.6s 停顿 → **1** | 60.8s 停顿 → **82**；90.8s 停顿 → **106** | 🔴 **阻塞越久错误越多**，须配合应用侧幂等重试 |
| **抖动期间 P99 延迟峰值** | 停顿期间 TPS 归零，p95 在恢复瞬间冲高至 **3.0 s** 量级 | 同样冲高至 **3.2 s** 量级 | ✅ 印证"调参不改变抖动本身"，两者一致 |
| **超阈值停顿是否仍被检出** | 5.8s 即检出并切主 | **90.8s 仍检出并切主**（T+61.18s，4/4 流） | ✅ **保护未被关闭，只是门槛平移** |
| 模拟真实坏盘（`dm-flakey` 报错）的检出时间 | _未测_ | _未测_ | 源码论据（错误次数路径不受 `tolerance_time` 影响）仍待实证，见 README §7.7.6 |

**原始数据**：`results/ladder-default.log`、`results/ladder_tuned.txt`、`results/ladder_tuned2.txt`、
`results/steady_default5.txt`、`results/steady_tuned60.txt`、`results/final.log`、
`results/timeline-default.txt`、`results/sb-default/`、`results/sb-tuned/`

### 8.3 基线性能实测（PSSDv2 默认 provisioning，fio 3.35） [实测]

| 场景 | 卷 | IOPS | 带宽 | p99.9 延迟 | 最大延迟 |
|---|---|---|---|---|---|
| 4k 随机读 | `/data/1`（6 盘条带） | 20.3k | 79.3 MiB/s | 202 ms | 577 ms |
| 4k 随机写 | `/data/1`（6 盘条带） | 18.4k | 72.0 MiB/s | 184 ms | 594 ms |
| 128k 顺序写 | `/data/1`（6 盘条带） | 6,481 | 810 MiB/s | 296 ms | 541 ms |
| 128k 顺序写 | `/data/log1`（4 盘条带） | 4,275 | 534 MiB/s | 447 ms | 725 ms |
| 4k 随机写 | `/data/log1`（4 盘条带） | 12.6k | 49.3 MiB/s | 692 ms | **1,141 ms** |

**两条关键判读**（详见 README §7.6.1）：

1. 实测吞吐 ≈ 盘数 × PSSDv2 出厂基线，**条带化只是在拼凑基线，并未突破**；
   6 盘条带仅达到 D32s v6 上限（66,667 IOPS）的约 30%，
   而**单块 PSSDv2 可 provision 到 80,000 IOPS** —— 条带布局既没换来性能，又放大了暴露面。
2. 卷被打满时，**尾延迟本身就已进入 OB 判定门槛的量级**（clog 卷峰值 1.14s，无任何注入故障）。
   OB 无法区分"限流排队"与"平台抖动"，因此 **provisioned 能力不足本身即为独立诱因**。

**原始数据**：`results/fio-baseline.txt`
