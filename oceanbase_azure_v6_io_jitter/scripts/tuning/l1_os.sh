...
apply() {
  ...
  if [[ -f /etc/default/grub ]]; then
    ...
    if [[ -n "${GRUB_CFG:-}" ]]; then
      grub2-mkconfig -o "$GRUB_CFG" >/dev/null 2>&1 && echo "    grub 已重新生成: $GRUB_CFG"
    else
      echo "    WARN: 未定位到 grub.cfg, 请手动执行 grub2-mkconfig"
    fi
    echo "    !! 需重启后内核命令行才生效 !!"
  else
    echo "    WARN: 无 /etc/default/grub, 请按发行版方式持久化"
  fi
  ...
  # --- 4. dirty 回写 ---
  echo
  echo "[4] vm.dirty_* 回写参数(降低脏页积压, 减小停顿期回写风暴)"
  cat > /etc/sysctl.d/99-ob-jitter.conf <<'EOF'
...
EOF
  sysctl -p /etc/sysctl.d/99-ob-jitter.conf | sed 's/^/    /'

  hr
  echo "完成。请重启使 nvme_core.io_timeout 内核命令行生效, 然后运行: $0 check"
}
...
rollback() {
  ...
  rm -f /etc/sysctl.d/99-ob-jitter.conf && echo "  已移除 /etc/sysctl.d/99-ob-jitter.conf"
  echo "  请手动重新生成 grub 并重启"
  hr
}
